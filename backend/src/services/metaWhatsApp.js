...
export class MetaWhatsAppService {
  ...
  saveConfig({ phoneNumberId, wabaId, accessToken, verifyToken, appSecret }) {
    ...
    const newConfig = {
      ...
      wabaId: wabaId || current.wabaId,
      verifyToken: verifyToken || current.verifyToken,
    };
    ...
    try {
      ...
      if (existing) {
        ...
        `).run(
          ...
          'Official Meta WhatsApp Business Cloud API for verified doctor outreach and ROI pitch dispatch',
          ts,
          'whatsapp',
          1
        );
      ...
      console.warn('[MetaWhatsAppService] DB save fallback:', err.message);
    }
  ...
  /**
   * Test connection by querying Phone Number info on Meta Graph API
   */
  ...
  async sendTextMessage({ to, text, previewUrl = true }) {
    ...
    if (!config.accessToken || !config.phoneNumberId) {
      throw new Error('Meta WhatsApp API is not configured. Please set Phone Number ID and Access Token.');
    }
    if (!to || !text) {
    ...
    logEvent({
      type: 'info',
      category: 'whatsapp',
      message: `Sending WhatsApp text message to +${recipient}`,
      meta: { to: recipient },
    });
    ...
    if (!res.ok) {
      ...
      logEvent({
        type: 'error',
        category: 'whatsapp',
        message: `Failed to send WhatsApp message to +${recipient}`,
        detail: errorMsg,
      });
  ...
  /**
   * Process incoming Webhook notifications from Meta
   */
    ...
    if (value?.statuses) {
      for (const status of value.statuses) {
        logEvent({
          type: 'info',
          category: 'whatsapp',
          message: `WhatsApp Status: Message ${status.id} -> ${status.status}`,
          meta: { status },
        });
      }
    }
    ...
    if (value?.messages) {
      for (const msg of value.messages) {
        logEvent({
          type: 'info',
          category: 'whatsapp',
          message: `WhatsApp Inbound Message from +${msg.from}: ${msg.text?.body || msg.type}`,
          meta: { from: msg.from, type: msg.type, text: msg.text?.body },
        });
      }
    }

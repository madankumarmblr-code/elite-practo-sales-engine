...
export async function syncSheetFromGoogle({ force = false } = {}) {
  if (syncing) return { ok: false, message: 'Sync already in progress', ...getSheetSyncStatus() };
  syncing = true;
  lastError = null;
  try {
    ensureDataDir();
    const res = await fetch(SHEET_CSV_URL, { headers: { Accept: 'text/csv,text/plain,*/*' }, redirect: 'follow' });
    if (!res.ok) throw new Error(`Google Sheet fetch failed: HTTP ${res.status}`);
    const text = await res.text();
    if (!text || !text.includes(',')) throw new Error('Google Sheet returned empty or invalid CSV');
    const lines = text.replace(/^\uFEFF/, '').split(/\r?\n/).filter((l) => l.trim());
    if (lines.length < 2) throw new Error('Google Sheet CSV has no data rows');

    fs.writeFileSync(CACHE_PATH, text.replace(/^\uFEFF/, ''), 'utf8');
    lastSync = new Date().toISOString();
    const meta = { lastSync, rows: lines.length - 1, bytes: Buffer.byteLength(text), sourceUrl: SHEET_CSV_URL, lastError: null };
    fs.writeFileSync(META_PATH, JSON.stringify(meta, null, 2));

    logEvent({ type: 'info', category: 'sheet', message: 'Google Sheet auto-synced', detail: `${meta.rows} rows cached`, meta });
    return { ok: true, message: 'Synced from Google Sheet', ...getSheetSyncStatus() };
  } catch (err) {
    lastError = err.message;
    ensureDataDir();
    const meta = { ...(fs.existsSync(META_PATH) ? JSON.parse(fs.readFileSync(META_PATH, 'utf8')) : {}), lastError, lastAttempt: new Date().toISOString() };
    fs.writeFileSync(META_PATH, JSON.stringify(meta, null, 2));
    logEvent({ type: 'error', category: 'sheet', message: 'Google Sheet sync failed', detail: err.message });
...
export function startSheetAutoSync() {
  ...
  syncTimer = setInterval(() => { syncSheetFromGoogle().catch(() => {}); }, SYNC_INTERVAL_MS);

...
export function logEvent({ type = 'info', category = 'system', message, detail = '', userId = null, meta = null }) {
  try {
    db.prepare(`
      INSERT INTO system_events (id, type, category, message, detail, user_id, meta, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(nanoid(), type, category, message, detail || '', userId, meta ? JSON.stringify(meta) : null, now());
  } catch (err) {
